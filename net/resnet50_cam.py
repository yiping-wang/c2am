...
class PreCAM(nn.Module):
    def __init__(self):
        super(PreCAM, self).__init__()
        self.resnet50 = resnet50.resnet50(
            pretrained=True, strides=(2, 2, 2, 1))
...
class ReNet(nn.Module):
    ...
    def __init__(self, stride=16, n_classes=20):
        ...
        if stride == 16:
            ...
            self.resnet50 = resnet50.resnet50(
                pretrained=True, strides=(2, 2, 1, 1), dilations=(1, 1, 2, 2))
            self.stage1 = nn.Sequential(self.resnet50.conv1, self.resnet50.bn1,
                                        self.resnet50.relu, self.resnet50.maxpool, self.resnet50.layer1)
        self.stage2 = nn.Sequential(self.resnet50.layer2)
        self.stage3 = nn.Sequential(self.resnet50.layer3)
...
class Net_CAM_Feature(ReNet):

    def __init__(self, stride=16, n_classes=20):
        super(Net_CAM_Feature, self).__init__(
            stride=stride, n_classes=n_classes)
...
class Class_Predictor(nn.Module):
    def __init__(self, num_classes, representation_size):
        ...
        self.classifier = nn.Conv2d(
            representation_size, num_classes, 1, bias=False)
    ...
    def forward(self, x, label):
        ...
        prediction = [self.classifier(
            y.unsqueeze(-1).unsqueeze(-1)).squeeze(-1).squeeze(-1) for y in feature_list]
        labels = [torch.nonzero(label[i]).squeeze(1)
