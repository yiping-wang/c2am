...
def load_image_label_from_xml(img_name, voc12_root):
    ...
    elem_list = minidom.parse(os.path.join(voc12_root, ANNOT_FOLDER_NAME, decode_int_filename(
...
class VOC12ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = np.asarray(imageio.imread(
            get_img_path(name_str, self.voc12_root)))
...
class VOC12ImageDatasetFD(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = torch.stack([img, torch.flip(img, (-1,))], dim=0)
        return {'name': name_str, 'img': img}


class VOC12ClassificationDSFD(VOC12ImageDatasetFD):
